...
// NOTE: these should probably be moved to the audio_utils.h file
#ifdef __arm__
  /* for ARMv6 and later */
  static inline uint32_t f_swahb32(uint32_t x) {
    __asm__ ("rev16 %0, %1" : "=r" (x) : "r" (x));
    return x;
  }
# define f_swab16(x) ((uint16_t)f_swahb32(x))
  static inline uint32_t f_swab32(uint32_t x) {
    __asm__ ("rev %0, %1" : "=r" (x) : "r" (x));
    return x;
  }
#define __Swap32 f_swab32
#define __Swap16 f_swab16
#else
  inline void __Swap16(uint16_t u) 
  {
    return (u << 8) | (u >> 8);
  }
  inline void __Swap32(uint32_t u)
  {
...
#define SWAP_U32_FROM_LITTLE(x) { x = __Swap32(x); }
#define SWAP_U16_FROM_LITTLE(x) { x = __Swap16(x); }
#else
